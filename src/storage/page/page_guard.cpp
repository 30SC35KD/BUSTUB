...
void ReadPageGuard::Flush() { //UNIMPLEMENTED("TODO(P1): Add implementation."); 
  ...
  //写入磁盘
  disk_scheduler_->Schedule(DiskRequest{true,frame_->GetDataMut(),page_id_,std::move(promise)});
  future.wait();
...
void WritePageGuard::Flush() { //UNIMPLEMENTED("TODO(P1): Add implementation."); 
  ...
  auto promise=disk_scheduler_->CreatePromise();
  auto future = promise.get_future();
  disk_scheduler_->Schedule(DiskRequest{true,GetDataMut(),page_id_,std::move(promise)});
  future.wait();
  frame_->is_dirty_=false;
  }
}
